...
export const configSchema = z.object({
  ...
  scanner: z.object({
    ...
    kol: z.object({
      ...
      maxEventsPerHourPerWallet: z.number().int().min(1),
    }),
  }),
  filter: z.object({
    creatorBlacklistFile: z.string(),
    migration: z.object({
      quoteMints: z.array(z.string()).min(1),
      minLiquidityUsd: z.number().min(0),
      minBuysM5: z.number().int().min(0),
      minBuyersM5: z.number().int().min(0),
      minBuySellRatioM5: z.number().min(0),
      minAgeForFlowRulesSec: z.number().int().min(0),
      requireAuthoritiesRevoked: z.boolean(),
      maxRugScoreNormalised: z.number().min(0).max(100),
      forbidDangerRisks: z.boolean(),
      maxTransferFeePct: z.number().min(0),
      maxCreatorDeadTokenRatio: z.number().min(0).max(1),
      minCreatorTokensForRatio: z.number().int().min(1),
    }),
    mature: z.object({
      quoteMints: z.array(z.string()).min(1),
      minLiquidityUsd: z.number().min(0),
      minVolumeH24Usd: z.number().min(0),
      minVolumeToLiquidity: z.number().min(0),
      minHolders: z.number().int().min(0),
      maxTop10HoldersPct: z.number().min(0).max(100),
      minLpLockedPct: z.number().min(0).max(100),
      lpBurnedDexIds: z.array(z.string()),
      maxRugScoreNormalised: z.number().min(0).max(100),
      forbidDangerRisks: z.boolean(),
      maxTransferFeePct: z.number().min(0),
      requireSignalTag: z.boolean(),
    }),
    signals: z.object({
      breakout: z.object({
        minChangeH1Pct: z.number(),
        minChangeM5Pct: z.number(),
        minVolumeH1ToHourlyAvg: z.number().min(0),
      }),
      goldenSwing: z.object({
        maxChangeH24Pct: z.number(),
        minChangeH1Pct: z.number(),
        minVolumeH1ToHourlyAvg: z.number().min(0),
      }),
    }),
  ...
  enrich: z.object({
    timeouts: z.object({
      ...
      securityMatureMs: z.number().int().positive(),
    }),
    cacheTtl: z.object({
      migrationSec: z.number().int().min(0),
      matureSec: z.number().int().min(0),
    }),
    solUsdcPair: z.string(),
    contextCacheSec: z.number().int().min(1),
    kolClusterWindowMin: z.number().int().min(1),
    geckoFlowOnMigration: z.boolean(),
    geckoCandlesOnMature: z.boolean(),
  }),
  risk: z.object({
  ...
  providers: z.object({
    ...
    geckoterminal: z.object({
      ...
      adaptiveMinPerMinute: z.number().positive(),
      timeoutMs: z.number().int().positive(),
    }),
    rugcheck: z.object({
      baseUrl: z.string().url(),
    ...
    websocket: z.object({
      ...
      maxMessagesPerMinutePerSubscription: z.number().int().positive(),
