...
| Launchpad graduations (push) | Alchemy WSS (`solana-mainnet.streaming.alchemy.com`, verified 2026-09-25) `logsSubscribe` on migration programs (pump.fun migration `39azUYFW…`, PumpSwap `pAMMBay6…` create_pool, Raydium LaunchLab `LanMV9sA…`, Meteora DBC `dbcij3LW…`, Boop, Moonshot/Moonit, Heaven); Helius WSS as second choice for low-volume subscriptions (byte-accounted at 20 credits/MB, ~1,200 credits/month for a migration stream); PumpPortal WSS `subscribeMigration` + `subscribeNewToken` (free, one connection); public RPC WSS last | Alchemy 30M CU; Helius 1M credits; PumpPortal free | subscribe only to migration/create-pool programs, never to swap programs; an endpoint that answers "Method not found" is skipped automatically |
