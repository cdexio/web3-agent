...
| **RugCheck** | https://fluxrpc.com (RugCheck moved API keys here; the legacy wallet login was removed) | `RUGCHECK_API_KEYS` | Anonymous 10 reports/min; authenticated 60/min per key | Sent in the `X-API-KEY` header (verified 2026-09-25 with the owner's key). Later the same day the key was rejected (`invalid api key`) on every endpoint, so free FluxRPC keys appear to carry a quota or expire; the engine falls back to anonymous automatically and cools the key down. |
