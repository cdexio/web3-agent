...
| **RugCheck** | https://fluxrpc.com (RugCheck moved API keys here; the legacy wallet login was removed) | `RUGCHECK_API_KEYS` | Anonymous 10 reports/min; authenticated 60/min per key | Sent in the `X-API-KEY` header (verified 2026-09-25 with the owner's key). The key is accepted on report endpoints but rejected (401) on the `/v1/stats/*` lists; the engine falls back to anonymous for those automatically. |
